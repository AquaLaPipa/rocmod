...
void G_KillBoxAdm (gentity_t *ent) {
	...
	for ( i = 0; i < num ; i++ ) 
	{
		...
		G_Damage ( hit, NULL, ent, NULL, NULL, 100000, DAMAGE_NO_PROTECTION|DAMAGE_FORCE_GORE, MOD_ADMIN_TELEFRAG, location );
	}
...
Sends an entity to a single client to play a sound
============
*/
void G_LocalSound( int clientNum, char *filename )
{
	...
	te = G_TempEntity( level.clients[clientNum].ps.origin, EV_GLOBAL_SOUND );
