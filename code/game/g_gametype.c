...
void gametype_trigger_touch ( gentity_t *self, gentity_t *other, trace_t *trace ) 
{
	...
	if ( trap_GT_SendEvent ( GTEV_TRIGGER_TOUCHED, level.time, self->health, other->s.number, other->client->sess.team, 0, 0 ) )
	{
		...
		if ( (BG_RelatedGametype( level.gametype, "ctf" ) || BG_RelatedGametype( level.gametype, "inf" ) || BG_RelatedGametype( level.gametype, "dem" )) )
		{
			if ( BG_RelatedGametype( level.gametype, "inf" ) )
			{
				itemname = "Briefcase";
				other->client->sess.modData->flagcaps++;
				G_LogPrintf("Briefcase Capture: %i %i: %s\n", other->client->sess.team, other->s.number, other->client->pers.netname );
			}
			else if ( BG_RelatedGametype( level.gametype, "dem" ) )
			{
				itemname = "Bomb";
			}
			else
    ...
    // END POINTS | Flag Capture - BuLLy
			...
			{
				AdminCmd_DoPenalty( other, level.time + (g_flagstealAutoPenaltyBoxTime.integer * 1000), 3 );
				G_Sound(other, CHAN_AUTO, G_SoundIndex( "sound/movers/doors/airlock_door02/airlock02_doorstop.mp3" ) );
				trap_SendServerCommand( -1, va("cp \"@%s^3 has been thrown into\nthe ^1penalty box^3!\n^7Reason: ^5%s stealing\n\"", other->client->pers.netname, itemname));
				trap_SendServerCommand( -1, va("print \"@%s^3 has been thrown into the ^1penalty box^3 for ^5%s stealing!\n\"", other->client->pers.netname, itemname));

				G_AddScore( other, -10 );		// remove the score he got from the flag

//				G_AddScore( other, -20 );		// remove the score he got from the flag, plus penalize him
//				other->client->sess.modData->recondata->score -= 20;
//				other->client->ps.persistant[PERS_SCORE] = other->client->sess.modData->recondata->score;
//				if ( other->s.number == level.redFlagKiller )
//				{
//					g_entities[level.redFlagMartyr].client->sess.modData->recondata->score += 10;
//					g_entities[level.redFlagMartyr].client->ps.persistant[PERS_SCORE] = g_entities[level.redFlagMartyr].client->sess.modData->recondata->score;
//				}
//				else if ( other->s.number == level.blueFlagKiller )
//				{
//					g_entities[level.blueFlagMartyr].client->sess.modData->recondata->score += 10;
//					g_entities[level.blueFlagMartyr].client->ps.persistant[PERS_SCORE] = g_entities[level.blueFlagMartyr].client->sess.modData->recondata->score;
//				}
//				CalculateRanks();
			}
...
void G_RespawnClients ( qboolean force, team_t team, qboolean fullRestart )
{
	...
	for ( i = 0; i < level.numConnectedClients; i ++ )
	{
		...
		// Dont respawn them if they are in the penalty box
		if ( ent->client->sess.modData->penalty )
		{
			continue;
		}

		// Dont respawn them if they are frozen admins
		if ( ent->client->sess.modData->adminref > 1 && ent->client->sess.modData->adminFreezeRespawn )
		{
			continue;
		}
		...
		if ( fullRestart )
		{
			ent->client->sess.modData->recondata->score = 0;
			ent->client->sess.modData->recondata->kills = 0;
			ent->client->sess.modData->recondata->deaths = 0;
			ent->client->sess.modData->hits = 0;
			ent->client->sess.modData->misses = 0;
			ent->client->sess.modData->headshots = 0;
			ent->client->sess.modData->flagcaps = 0;
...
void G_ResetGametype ( qboolean fullRestart )
{
	...
	if ( level.intermissionQueued && !sv_modClient.integer )
	{
		return;
	}
	...
	switch ( level.gametypeData->respawnType )
	{
		...
		case RT_NONE:
			...
			memset ( &level.teamAlivePrinted[0], 0, sizeof(level.teamAlivePrinted) );
...
void CheckGametype ( void )
{
	...
	if ( level.intermissiontime || (level.intermissionQueued && !sv_modClient.integer && level.time - level.intermissionQueued >= INTERMISSION_DELAY_TIME) )
	{
		return;
	}
	...
	if ( level.gametypeData->respawnType == RT_NONE && level.gametypeStartTime )
	{
		...
		memset ( &level.teamAliveClient[0], 0, sizeof(level.teamAliveClient) );
		...
		for ( i = 0; i < level.numConnectedClients; i ++ )
		{
			...
			else
			{
				...
				level.teamAliveClient[ent->client->sess.team] = ent->s.number;
			}
		}

		if ( level.time > level.gametypeDelayTime )
		{
			if ( !level.teamAlivePrinted[TEAM_RED] && level.teamAliveCount[TEAM_RED] == 1 && dead[TEAM_RED] )
			{
				level.teamAlivePrinted[TEAM_RED] = qtrue;
				trap_SendServerCommand( level.teamAliveClient[TEAM_RED], "cp \"^3You are the last player alive on your team!\n\"" );
				if ( voicecmds.voicePromptSound[7][0] )
				{
					G_Sound(g_entities + level.teamAliveClient[TEAM_RED], CHAN_AUTO, G_SoundIndex( voicecmds.voicePromptSound[7] ) );
				}
			}
			else if ( !level.teamAlivePrinted[TEAM_BLUE] && level.teamAliveCount[TEAM_BLUE] == 1 && dead[TEAM_BLUE] )
			{
				level.teamAlivePrinted[TEAM_BLUE] = qtrue;
				trap_SendServerCommand( level.teamAliveClient[TEAM_BLUE], "cp \"^3You are the last player alive on your team!\n\"" );
				if ( voicecmds.voicePromptSound[7][0] )
				{
...
{
	trap_GT_SendEvent ( GTEV_ENDGAME, level.time, 0, 0, 0, 0, 0 );
}
