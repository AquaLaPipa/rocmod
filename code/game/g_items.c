...
void Touch_Item (gentity_t *ent, gentity_t *other, trace_t *trace) 
{
	...
	if ( ent->item->giType == IT_GAMETYPE )
	{
		...
		if ( BG_RelatedGametype( level.gametype, "ctf" ) || BG_RelatedGametype( level.gametype, "inf" ) || BG_RelatedGametype( level.gametype, "dem" ) )
		{
			...
			if ( other->s.number == level.redFlagKiller && other->client->sess.modData->adminref < 2 )
			{
				if ( g_flagstealWarning.integer )
				{
					trap_SendServerCommand( other-g_entities, va("cp \"\n^1No Stealing!\n^3That %s belongs to %s%s^3!\n^1You better not capture it!\n\"", itemname, level.teamData.teamcolor[2], g_entities[level.redFlagMartyr].client->pers.netname) );
				}
			}
			else if ( other->s.number == level.blueFlagKiller && other->client->sess.modData->adminref < 2 )
			{
				if ( g_flagstealWarning.integer )
				{
					trap_SendServerCommand( other-g_entities, va("cp \"\n^1No Stealing!\n^3That %s belongs to %s%s^3!\n^1You better not capture it!\n\"", itemname, level.teamData.teamcolor[1], g_entities[level.blueFlagMartyr].client->pers.netname) );
				}
			}
			else if ( other->client->sess.team == TEAM_RED )
			{
	...
	if ( ent->item->giTag != other->client->sess.modData->pickupItem || level.time - other->client->sess.modData->pickupTime > 1000 )
	{
		other->client->sess.modData->pickupItem = ent->item->giTag;
		other->client->sess.modData->pickupTime = level.time;
...
gentity_t *G_DropItem( gentity_t *ent, gitem_t *item, float angle )
{
	...
	if ( item->giType == IT_GAMETYPE )
	{
		trap_GT_SendEvent ( GTEV_ITEM_DROPPED, level.time, level.gametypeItems[item->giTag].id, ent->s.number, ent->client->sess.team, 0, 0 );
		...
		if (!ent->culprit || !ent->culprit->client)
		{
			ent->culprit = ent;
		}
		...
		G_LogPrintf("(%i) %s drops %s, by (%i) %s, onsameteam:%i\n",
			ent->s.number, ent->client->pers.netname, item->classname,
